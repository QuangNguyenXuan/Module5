...
# Training settings
parser = argparse.ArgumentParser(description='AI VietNam All-in-One Course')
...
parser.add_argument('--dataset', type=str, default='office', help='the name of dataset')
...
record_file = os.path.join(record_dir, '%s_net_%s_%s' % (args.method, args.net, args.data))

labeled_data_loader, unlabeled_data_loader_test, class_list = return_dataset(args)
...
def train():
    ...
    optimizer_gcn = optim.SGD(list(GCN.parameters()), lr=1.0, momentum=0.9, weight_decay=0.0005, nesterov=True)
    optimizer_f = optim.SGD(list(MLP_Classifier.parameters()), lr=1.0, momentum=0.9, weight_decay=0.0005, nesterov=True)
    ...
    for step in range(all_step):
        ...
        optimizer_gcn = inv_lr_scheduler(param_lr_gcn, optimizer_gcn, step, init_lr=args.lr)
        ...
        gt_labels_labeled.resize_(data_labeled[1].size()).copy_(data_labeled[1])
        ...
        if step % args.save_interval == 0 and step > 0:
            ...
            if args.save_check:
                print('saving model')
                torch.save(G.state_dict(), os.path.join(args.checkpath, "G_iter_model_{}_{}_step_{}.pth.tar".format(args.method, args.data, step)))
                torch.save(MLP_Classifier.state_dict(), os.path.join(args.checkpath, "MLP_Classifier_iter_model_{}_{}_step_{}.pth.tar".format(args.method, args.data, step)))
